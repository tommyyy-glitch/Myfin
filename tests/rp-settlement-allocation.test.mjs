...
const code=[
  ...
  `
    const S={
      ...
      txns:[],accounts:[{id:'bank',label:'Bank',icon:'🏦'}],_settleIds:null,_settleId:null,_settleAcct:null,_editSettleTxnId:null
    };
    const document={
    ...
    S._settleId='lunch';S._settleIds=['lunch'];S._settleAcct='bank';S._editSettleTxnId=firstTxn.id;
    ...
    globalThis.result={initial,afterBlocked,afterFive,afterEdit,afterDelete:{lunch:S.people[0].amtHKD,salary:S.people[1].amtHKD,count:S.txns.filter(tx=>tx.rpPhase==='settle').length},toasts};
  `
].join('\n');
...
assert.doesNotMatch(declaration('confirmSettle'),/items\.forEach/);
assert.match(html,/data-i="applytorecord"/);
assert.match(html,/app will not auto-allocate the payment/);

console.log('Explicit per-record R/P settlement allocation tests passed.');
